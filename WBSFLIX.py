...
def popularity_recommender(num_recommendations):
  try:
      ...
      movie_data['ratings'] = calculate_popularity(movie_data)
      sorted_movies = movie_data.sort_values(by='ratings', ascending=False)
...
def calculate_popularity(movie_data):
  average_ratings = movie_data.groupby('movieId')['ratings'].mean()
  popularity_score = (average_ratings * np.log1p(movie_data['ratings'].count())).fillna(0)
